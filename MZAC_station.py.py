# -*- coding: utf-8 -*-
"""
Created on Sat Oct 22 01:20:36 2022

@author: diego
"""
#Hola carepankei, hay muchas grafica como comentario xq iba probando de a poco pero si ejecutas te salen las principales
#cualquier cambio,correcion,etc bienvenido
#cosas por mejorar, definición y ajuste de gráficos(que se vean mas fechas en residuales por ej), ubicación de legend, colores maybe 
...
datos =extract_data("MZAC.txt", False) # toma todos los datos como un array
...
#Ahora vamos con parte ESTACIONAL
...
t_illapel=toYearFraction(datetime(2010,2,27))
x=np.where(t == t_illapel)

t2=toYearFraction(datetime(2015,9,16))
x2=np.where(t == t2)
...
for i in range(t.shape[0]):
    if i>=1762:   # el uno es la posicion de mi salto 
        h1[i]=1
       #primer salto posicion 1695
   # segundo salto 2261
   
#nose como buscar otro x(array)para ver el otro salto de illapel
   
#t_illapell=toYearFraction(datetime(2015,9,16))
#x=np.where(t == t_illapell)

h2=np.zeros((t.shape[0],1))
for i in range(t.shape[0]):
     if i>=3717:   # el uno es la posicion de mi salto 
         h2[i]=1
#        #primer salto posicion 1695
   # segundo salto 2261
...
#Vamos con el LOGARITMO (post-sismico)
...
for i in range(t.shape[0]):
    if i>=1762:   # el uno es la posicion de mi salto
        tau=0.00001/365.25 #con 10 funciona perfect, con 1 funciona mjr, mientras mas pequeño el numerador crece pick de N-S
        l1[i]=np.log(1+(t[i]-t[1762])/tau)

l2=np.zeros((t.shape[0],1))
for i in range(t.shape[0]):
    if i>=3717:   # el uno es la posicion de mi salto
        tau=2/365.25 #con 10 funciona perfect, con 1 funciona mjr, mientras mas pequeño el numerador crece pick de N-S
        l2[i]=np.log(1+(t[i]-t[3717])/tau)
                     
                     
G_post=np.concatenate((l1,l2),axis=1)
...
fig, axs = pl.subplots(3)
fig.suptitle('Estación MZAC',fontsize=14)
...
#cambio de antenas
init_xa =toYearFraction(datetime(2009,12,10))
init_xb =toYearFraction(datetime(2011,12,19))
init_xc =toYearFraction(datetime(2016,11,15))
init_xd =toYearFraction(datetime(2017,8,15))
#terremotos 
init_xe =toYearFraction(datetime(2010,2,27))
init_xf =toYearFraction(datetime(2015,9,27))

axs[0].axvline(init_xa, color="c", linestyle="--")
axs[0].axvline(init_xb, color="c", linestyle="--")
axs[0].axvline(init_xc, color="c", linestyle="--")
axs[0].axvline(init_xd, color="c", linestyle="--")

axs[0].axvline(init_xe, color="grey", linestyle="--")
axs[0].axvline(init_xf, color="grey", linestyle="--")
...
axs[0].plot(t_illapel,0, ms=6, mfc='c',mec='c',color='c',label='Cambios de antena')
axs[0].plot(t_illapel,0, ms=6, mfc='c',mec='c',color='grey',label='Terremotos')
...
# en axs[0] ploteamos primera columna de datos con la fecha que son los años
axs[1].axvline(init_xa, color="c", linestyle="--")
axs[1].axvline(init_xb, color="c", linestyle="--")
axs[1].axvline(init_xc, color="c", linestyle="--")
axs[1].axvline(init_xd, color="c", linestyle="--")

axs[1].axvline(init_xe, color="grey", linestyle="--")
axs[1].axvline(init_xf, color="grey", linestyle="--")
...
axs[2].axvline(init_xa, color="c", linestyle="--")
axs[2].axvline(init_xb, color="c", linestyle="--")
axs[2].axvline(init_xc, color="c", linestyle="--")
axs[2].axvline(init_xd, color="c", linestyle="--")

axs[2].axvline(init_xe, color="grey", linestyle="--")
axs[2].axvline(init_xf, color="grey", linestyle="--")
