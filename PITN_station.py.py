...
@author: diego
"""
#Hola caremaipu espero que estes bien aqui esta la estacion de brasil du brasiiullll siuuuuu falao puta aregao muito pennis 
...
datos =extract_data("PITN.txt", False) # toma todos los datos como un array
# print(datos['data'][:,1]) #primera fila de matriz


## PRIMERA FUNCION  MODELO LINEAL
...
#SEGUNDA FUNCION MODELO ESTACIONAL

t=datos['data'][:,0] # es lo mismo que c1 de mas arriba pero lo cambio a t 
...
#TERCERA FUNCION MODELO DE SALTOS

#aqui debemos saber la fecha exacta del sismo o cambio de antena
t_illapel=toYearFraction(datetime(2019,11,13))
...
for i in range(t.shape[0]):
    if i>=3203:   # el uno es la posicion de mi salto 
        h1[i]=1
...
for i in range(t.shape[0]):
    if i>=3203:   # el uno es la posicion de mi salto
        ...
        l1[i]=np.log(1+(t[i]-t[3203])/tau)
#aqui coloque 3203 porque hay un cambio de estacion 
#cambio en el modelo de trayectoria 

G_post=l1
#TENEMOS LAS COMPONENTES LOGARITMICAS
...
fig.suptitle('Estación PITN',fontsize=14)
...
#CODIGO PARA PODER HACER LAS LINEAS DISCONTINUAS DE LOS CAMBIOS DE ANTENA
init_xa =toYearFraction(datetime(2019,11,13))
...
axs[0].plot(init_xa,0,linestyle="--", ms=6, mfc='c',mec='c',label='Cambio de antena')
axs[0].set_ylabel('E-W [mm]',fontsize=9)
