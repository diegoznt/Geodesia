# -*- coding: utf-8 -*-
"""
Created on Tue Oct 25 11:11:29 2022
...
datos =extract_data("SLMC.txt", False) # toma todos los datos como un array
...
for i in range(t.shape[0]):
    if i>=962:   # el uno es la posicion de mi salto 
        h1[i]=1
...
for i in range(t.shape[0]):
    if i>=962:   # el uno es la posicion de mi salto
        tau=55/365.25 #con 10 funciona perfect, con 1 funciona mjr, mientras mas pequeño el numerador crece pick de N-S
        l1[i]=np.log(1+(t[i]-t[962])/tau)
...
fig, axs = pl.subplots(3)
fig.suptitle('Estación SLMC',fontsize=14)
...
init_xb =toYearFraction(datetime(2015,11,11))
init_xc =toYearFraction(datetime(2017,6,12))

axs[0].axvline(init_xa, color="grey", linestyle="--")
axs[0].axvline(init_xb, color="grey", linestyle="--")
axs[0].axvline(init_xc, color="grey", linestyle="--")
...
axs[1].axvline(init_xc, color="grey", linestyle="--")
...
axs[2].axvline(init_xc, color="grey", linestyle="--")
...
pl.tight_layout()
pl.figure(figsize=(12, 12), dpi=90)
...
axs[1,0].set_ylabel('N-S [mm]',fontsize=9)
...
axs[2,0].set_ylabel('Up [mm]',fontsize=9)
...
axs[1,1].set_ylabel('N-S [mm]',fontsize=9)
...
axs[2,1].set_ylabel('Up [mm]',fontsize=9)
...
#axs[2].set_title('Ajuste Modelo de Trayectoria [Up]',fontsize=10)
pl.tight_layout()
pl.figure(figsize=(15, 12), dpi=90)
